...
@section('script')
    <script>
        new Vue({
            ...
            try {
                this.isLoading=true
                const res = await axios.get(`/api/link/visit/stats/${this.link_id}/${this.user_id}`,
                ...
                if(res.data.satus='success'){
                    ...
                    data.clicks.forEach(item => {
                        this.months.push(item.month)
                        this.clicks_count.push(item.link_click_count)
                    });
